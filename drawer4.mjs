...
for (let i=0;i<3;i++){
  ...
  const open = await p.evaluate(()=>[...document.querySelectorAll('div')].some(n=>typeof n.className==='string'&&/bg-cover/.test(n.className)));
  console.log(`click ${i+1} -> open=${open}`);
  if(open) break;
}
const st = await p.evaluate(()=>{
  ...
  return { bg:d?getComputedStyle(d).backgroundImage:null,
