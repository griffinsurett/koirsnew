...
export const iconSchema = ({ image }: { image: Function }) =>
  z.union([
    // `image()` MUST come before `z.string()`.
    //
    // Zod unions match in declaration order. With `z.string()` first, an asset
    // path like "@/assets/foo.png" satisfies the string branch immediately, so
    // `image()` never runs and the value stays a raw string — which then
    // reaches renderIcon() and is treated as a library token ("lu:home"),
    // matches nothing, and renders NOTHING. Silently: no error, no warning.
    //
    // The result was that image icons could not work at all, even though the
    // union advertised support for them and renderObjectIcon() had a working
    // `'src' in icon` branch waiting. imageInputSchema (above) already gets
    // this ordering right; this schema simply had it backwards.
    //
    // Library tokens still work: they don't resolve as assets, so image()
    // rejects them and the string branch below catches them as before.
    image(),
    z.string(),
    z.object({
...
/**
 * Per-page CTA copy, available on both items (`baseSchema`) and collection
 * indexes (`metaSchema`). Lets a page's frontmatter drive its CTA banner
 * instead of the banner hardcoding copy or taking it from props at every
 * call site.
 *
 * The headline is split so one word can be emphasized mid-line:
 *   title: "Protect your"  accent: "roof"  titleAfter: "before winter."
 */
...
export const ctaSchema = z
  .object({
    ...
    accent: z.string().optional(),     // the emphasized word
    titleAfter: z.string().optional(), // text after the accent word
    description: z.string().optional(),
    // Explicit benefit list. Omit and the banner falls back to a collection
    // (e.g. benefits), limited by `count`.
...
export const metaSchema = ({ image }: { image: Function }) =>
  z.object({
    ...
    featuredImage: imageInputSchema({ image }).optional(),
    // Hero banner photo for the collection index (behind the service hero).
