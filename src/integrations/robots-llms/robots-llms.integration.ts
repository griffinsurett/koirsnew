...
function readCollection(collectionDir: string, collectionName: string): CollectionData | null {
  ...
  const dirFiles = readdirSync(collectionDir);
  ...
  // JSON-backed collections (FileLoad rather than GlobLoad).
  //
  // Without this, a collection whose entries live in a single .json array is
  // invisible here: the .mdx filter above matches nothing, `items` stays empty,
  // and the `!items.length` guard below drops the whole collection. Since this
  // "Supporting Content" section is the ONLY route into llms-full.txt for
  // page-less collections, that silently omitted every JSON entry on the site.
  //
  // JSON entries have no frontmatter and no MDX body, so extractMdxBody is not
  // applicable — the fields map directly instead. `id` is the slug (same
  // convention the content scanner and the schema layer use).
  if (items.length === 0) {
    const jsonFiles = dirFiles.filter((f) => f.endsWith('.json') && !f.startsWith('_')).sort();

    for (const file of jsonFiles) {
      let entries: unknown[];
      try {
        const parsed = JSON.parse(readFileSync(join(collectionDir, file), 'utf8'));
        entries = Array.isArray(parsed) ? parsed : Object.values(parsed);
      } catch {
        // Malformed JSON is reported with better context by the schema layer.
        continue;
      }
      ...
      for (const entry of entries) {
        ...
        if (!e.title) continue;
        // Per-entry opt-out, honored the same way as MDX frontmatter.
        if (e.llms?.addToLLMs === false) continue;

        items.push({
          slug: String(e.id ?? e.slug ?? ''),
          title: e.title,
          description: e.description,
          features: Array.isArray(e.features) ? e.features : undefined,
        });
      }
    }
  }
